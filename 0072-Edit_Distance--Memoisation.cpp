 ...
 * But practically on LeetCode, memoisation seems to be slower (60ms) than dynamic
 * programming (10ms). I presume this is because memoisation had additional
 * overhead from the recursive function calls.
 *
 * Additionally, memoisation has worse space complexity at O(m*n) as compared to
 * dynamic programming at O(m). This is because it doesn't rely on the substructure
 * of the problem and thus has to remember all the subproblems instead of a subset.
...
public:
     ...
     *
     * @params word1 word to convert into word2
     * @params word2 word to convert into word1
     * @returns mimimum number of operations to convert word1 into word2
     */
